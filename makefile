env=dev
docker-os=
compose=docker-compose -f docker-compose.yml -f docker/$(env)/docker-compose.yml

ifeq ($(docker-os), windows)
	ifeq ($(env), dev)
		compose += -f docker/dev/docker-compose.windows.yml
	endif
endif
...
artifact: ## build production artifact
		docker-compose -f docker/artifact/docker-compose.yml build
...
db: ## recreate database
		...
		$(compose) exec -T php sh -lc './bin/console d:m:m -n --allow-no-migration'
.PHONY: dmd
